...
def supervise(config_path:Path)->None:
    ...
    if all((job_dir(out,seed,cid)/"OUTPUT_COMPLETE.json").exists() for seed,cid in jobs):return
...
def contrasts(config_path:Path)->None:
    ...
    contrast_fields=["row_type","contrast","formula","metric","seed","delta","seed_sd","favorable_seed_count","bootstrap_low","bootstrap_high","evidence_language"]
    with PUBLIC_CONTRASTS.open("w",newline="",encoding="utf-8") as f:w=csv.DictWriter(f,fieldnames=contrast_fields);w.writeheader();w.writerows(rows)
...
def summarize(config_path:Path)->None:
    ...
    diag_mean={cid:{key:float(np.mean([r[key] for r in diag if r["condition_id"]==cid])) for key in ("actor_tanh_saturation_fraction","diagnostic_action_clip_fraction")} for cid in ("P0S0","P1S0","P0S1","P1S1")}
    seed_contrasts=[r for r in contrast["rows"] if r["row_type"]=="seed"]
    def window_result(name:str,metric:str)->tuple[float,int]:
        selected=[r["delta"] for r in seed_contrasts if r["contrast"]==name and r["metric"]==metric]
        return float(np.mean(selected)),sum(x<0 for x in selected)
    factor_by_name={r["field_name"]:r for r in factor["day03_comparison"]}
    lines=["# Day 04 장기 구간 확인 결과","","## 결론","",f"P0와 P1의 S1−S0 state contrast는 모두 `moderate_suggestive`였다(P0 {c['P0_state']['delta']:+.4f}, {c['P0_state']['favorable_seed_count']}/5 seeds; P1 {c['P1_state']['delta']:+.4f}, {c['P1_state']['favorable_seed_count']}/5 seeds). 평균 방향은 S1에 유리했지만 두 기술적 bootstrap 구간 모두 0을 포함하므로 강한 내부 견고성으로 부르지 않는다. 전처리 효과와 interaction은 seed 간 이질성이 있어 서술적으로만 해석한다.","","> 범위: 고정 환자 profile과 remifentanil schedule을 사용한 VitalDB-informed 재구성 simulation trajectory. 임상 효과, 외부 검증 또는 인과 효과의 증거가 아니다.","","## Primary conditions","","| Condition | MAE 0–1800 (mean ± seed SD) | MAE 0–600 | MAE 600–1800 | visibility | action boundary |","|---|---:|---:|---:|---:|---:|"]
    ...
    lines += ["","## 구간별 state 효과","","| Contrast | 0–600 s mean (favorable seeds) | 600–1,800 s mean (favorable seeds) |","|---|---:|---:|"]
    for name in ("P0_state","P1_state"):
        early=window_result(name,"mae_0_600");late=window_result(name,"mae_600_1800");lines.append(f"| {name} | {early[0]:+.4f} ({early[1]}/5) | {late[0]:+.4f} ({late[1]}/5) |")
    lines += ["", "평균 개선 방향은 초기와 후기 구간에 모두 나타났지만 seed 부호 일관성은 표와 같이 제한적이다. 후기 MAE가 모든 조건에서 초기보다 낮아, 1,800초 전체 차이는 초기 유도 구간의 영향도 함께 포함한다.","","## 관측 가용성과 정책 진단","","| Condition | visibility | SQI rejection | stale | actor Tanh saturation | diagnostic clip | core clip |","|---|---:|---:|---:|---:|---:|---:|"]
    for cid in ("P0S0","P1S0","P0S1","P1S1"):
        r=p[cid];d=diag_mean[cid];lines.append(f"| {cid} | {r['visible_fraction_mean']:.4f} | {r['sqi_rejection_fraction_mean']:.4f} | {r['stale_fraction_mean']:.4f} | {d['actor_tanh_saturation_fraction']:.2e} | {d['diagnostic_action_clip_fraction']:.4f} | {r['core_clip_fraction_mean']:.4f} |")
    lines += ["",f"P1 visibility는 P0보다 약 {100*(p['P0S0']['visible_fraction_mean']-p['P1S0']['visible_fraction_mean']):.1f}%p 낮았고, 차이는 SQI rejection과 거의 일치했다. actor Tanh saturation과 core clipping은 사실상 0이었으나 S1의 action-boundary/diagnostic-clip 비율은 약 3%로 S0보다 높았다.","","## Baseline 비교","","| Profile | constant | P | PI | matched S0 PPO | matched S1 PPO |","|---|---:|---:|---:|---:|---:|",f"| P0 | {baseline[('P0','constant')]['mae_0_1800']:.4f} | {baseline[('P0','P')]['mae_0_1800']:.4f} | {baseline[('P0','PI')]['mae_0_1800']:.4f} | {p['P0S0']['mae_0_1800_mean']:.4f} | {p['P0S1']['mae_0_1800_mean']:.4f} |",f"| P1 | {baseline[('P1','constant')]['mae_0_1800']:.4f} | {baseline[('P1','P')]['mae_0_1800']:.4f} | {baseline[('P1','PI')]['mae_0_1800']:.4f} | {p['P1S0']['mae_0_1800_mean']:.4f} | {p['P1S1']['mae_0_1800_mean']:.4f} |","", "S0 PPO 평균은 각 profile의 tuned PI보다 나빴고, S1 PPO 평균은 tuned PI보다 낮은 MAE를 보였다. 이는 동일 simulation 설정의 내부 비교이며 임상 우월성 주장이 아니다.","","## 장기 Nscale과 Day 03 transfer","",f"1,800초 train-only calibration은 1,209,600 observations를 사용했다. Day 03 대비 propofol cumulative-dose factor는 {factor_by_name['propofol_cumulative_dose_mg']['day03_factor']:.2f}→{factor_by_name['propofol_cumulative_dose_mg']['day04_factor']:.2f} ({factor_by_name['propofol_cumulative_dose_mg']['ratio']:.2f}×)로 커졌고, 네 concentration factor 비율은 1.06–1.43×였다. binary/mask 좌표는 1.0을 유지했고 BIS 순서와 S0-prefix 공유 검사를 통과했다.",f"Frozen Day 03 P0 S1−S0의 fresh-subset 1,800초 delta는 " + ", ".join(f"{x:+.4f}" for x in transfer_delta)+"로 3/3 모두 S1 방향이었다. 다만 legacy seed가 3개이고 training universe와 horizon이 동시에 달라졌으므로 사전 정의된 5-seed evidence label의 대상이 아니며, 방향성 transfer 진단으로만 본다.","","## Day 05에 대한 시사점","","- Feature-group ablation은 S1의 후기 정보(누적 dose와 effect-site/plasma concentration)를 우선 분리하되, S1에서 약 3%였던 action-boundary/clip 진단을 guardrail로 함께 추적해야 한다.","- 더 넓은 P-profile 연구에서는 train-only audit가 만든 뚜렷한 정보 regime을 사용한다. 0–1,800초 visibility는 SQI off/A30 0.575, SQI30/A30 0.453, SQI50/A30 0.439, SQI70/A30 0.381이었다. threshold 효과를 우선 확장하고, age 효과는 높은 threshold에서 별도 factorial로 확인한다.","- 이 제안들은 다음 연구 설계 입력일 뿐 Day 04 결과로 추가 PPO를 선택하거나 시작하지 않는다.","","## 한계","","내부 source cohort를 재사용했지만 이번 96-subject subset은 이전 journal 개발에 사용되지 않았다. 알려진 historical test 결과는 사용하지 않았고 original-test access는 0이었다. 재구성 simulation이며 임상·인과 주장을 하지 않는다. policy seed가 5개뿐이므로 계층 bootstrap은 기술적 민감도 구간이다.","","## 재현","",f"실효 budget은 job당 {protocol['effective_timesteps']:,} steps, 총 20 jobs/10,485,760 steps이며 복구 60개와 최종 20개 checkpoint를 검증했다. Worker는 1개, Torch thread는 1개였고 최초 준비부터 최종 산출물까지 wall time은 {timing.get('wall_hours',float('nan')):.2f}시간이었다. Resume: `.venv-journal\\Scripts\\python.exe scripts\\journal\\run_day04_confirmation.py resume`. Private identifiers, models, checkpoints, logs와 row-level 자료는 ignored Day 04 output root에만 있다."]
    PUBLIC_REPORT.write_text("\n".join(lines)+"\n",encoding="utf-8")
    PUBLIC_OVERLEAF.write_text("# Day 04 Overleaf note\n\n- Setting: VitalDB-informed reconstructed PK/PD simulation; no clinical, causal, or external-validation claim.\n- Design: full development-training partition (1,642 subjects/1,680 cases), fresh internal validation (96/96), 1,800-s horizon, five paired seeds, 524,288 steps per job.\n- Primary MAE (mean ± seed SD): "+"; ".join(f"{cid} {p[cid]['mae_0_1800_mean']:.4f} ± {p[cid]['mae_0_1800_seed_sd']:.4f}" for cid in ("P0S0","P1S0","P0S1","P1S1"))+".\n- Contrasts: "+"; ".join(f"{name} {c[name]['delta']:+.4f} ({c[name]['favorable_seed_count']}/5 favorable; [{c[name]['bootstrap_low']:+.4f}, {c[name]['bootstrap_high']:+.4f}])" for name in CONTRAST_FORMULAS)+".\n- State-effect language: P0 and P1 are moderate/suggestive; neither descriptive hierarchical bootstrap interval excludes zero.\n- Visibility: P0 0.5441, P1 0.4129; primary actor inputs were finite, Tanh saturation was negligible, and core clipping was zero.\n- Baseline PI MAE: P0 6.0052; P1 6.9496. Frozen Day 03 transfer deltas were -3.4645, -5.5104, and -1.4427 (descriptive only).\n- Descriptive hierarchical bootstrap resampled paired seeds and subjects; n=5 policy seeds; original-test access was zero.\n",encoding="utf-8")
...
def resume(config_path:Path)->None:
    config,_=load_config(config_path);out=out_root(config);start=time.time();timing_path=out/"timing.json";prior=json.loads(timing_path.read_text(encoding="utf-8")) if timing_path.exists() else {};started_utc=prior.get("started_utc",utc_now());atomic_json(timing_path,{"started_utc":started_utc})
    ...
    completed_utc=utc_now();elapsed=(datetime.fromisoformat(completed_utc)-datetime.fromisoformat(started_utc)).total_seconds()/3600;timing={"started_utc":started_utc,"completed_utc":completed_utc,"wall_hours":elapsed,"resume_invocation_hours":(time.time()-start)/3600};atomic_json(timing_path,timing);summarize(config_path);verify(config_path)
