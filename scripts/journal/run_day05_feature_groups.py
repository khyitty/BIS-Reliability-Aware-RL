...
def summarize(config_path: Path) -> None:
    ...
    lines=["# Day 05 feature-group 개발 스크리닝 결과","","## 결론","",f"Pareto non-dominated states: {', '.join(r['state'] for r in diagnosis['pareto'] if r['non_dominated'])}. 다음 개발 후보는 {', '.join(diagnosis['recommended_candidates'])}이다. 이는 재구성 simulation의 개발 추천이며 임상적 최적 상태나 생물학적 인과 효과가 아니다.","","## 상태별 최종 MAE","","| State | P0 mean ± SD (worst) | P1 mean ± SD (worst) | Extra features |","|---|---:|---:|---:|"]
    ...
    s50=[r for r in agg["rows"] if r["seed"]==50];lines += ["","## Seed 50 prespecified diagnostic","","| State | P0 MAE / propofol / in-range | P1 MAE / propofol / in-range |","|---|---:|---:|"]
    ...
    lines += ["","## Guardrails and interpretation","","Per-state/profile action-boundary, core-clipping, Tanh-saturation, raw/physical action quantiles, scaled group magnitudes, and all four checkpoint trajectories are in the public CSV/JSON artifacts. Validation used only final checkpoints and selected no checkpoint.","",f"Untouched future internal-confirmation reserve: {protocol['future_reserve_subject_count']} subjects; membership identifiers were hashed, bundle access 0, outcome access 0. Original-test access remained 0.","","## Neural Networks novelty implication","", "If enriched states share the same seed-50 failure, feature content alone is insufficient and the next protocol should test a genuinely new group-structured stability method. If a compact group removes the failure while retaining mean performance, broad Cp/Ce or drug-specific splitting is the next development ablation. No follow-up is launched here.","","## Limitations","","The Day 04 development-validation subset was reused after inspection, so this is exploratory screening rather than confirmation. The source cohort is internal and reused. This is reconstructed simulation, not clinical, causal, bedside-dosing, or external-validation evidence.","","## Reproduction","",f"30 new jobs, 15,728,640 new timesteps, 120 new checkpoints; one worker/thread. Wall time: {timing.get('wall_hours',float('nan')):.2f} h. Resume: `.venv-journal\\Scripts\\python.exe scripts\\journal\\run_day05_feature_groups.py resume`."]
    PUBLIC_REPORT.write_text("\n".join(lines)+"\n",encoding="utf-8")
    PUBLIC_OVERLEAF.write_text("# Day 05 Overleaf note\n\n- Exploratory development screening in reconstructed PK/PD simulation; not clinical, causal, or external-validation evidence.\n- States: S0, S_CUM, S_CONC, S_CORE, S1; P0/P1; five paired seeds; 1,800 s; 524,288 steps per new job.\n- MAE mean ± seed SD (P0 / P1): "+"; ".join(f"{s} {by[('P0',s)]['mean_mae']:.4f} ± {by[('P0',s)]['seed_sd']:.4f} / {by[('P1',s)]['mean_mae']:.4f} ± {by[('P1',s)]['seed_sd']:.4f}" for s in config["states"])+".\n- Pareto set: "+", ".join(r["state"] for r in diagnosis["pareto"] if r["non_dominated"])+"; development candidates: "+", ".join(diagnosis["recommended_candidates"])+".\n- Future reserve: 170 subjects, zero bundle/outcome access; original-test access zero.\n",encoding="utf-8")
...
def verify(config_path: Path) -> None:
    ...
    public=[PUBLIC_RESULTS,PUBLIC_CONTRASTS,PUBLIC_TRAJECTORY,PUBLIC_DIAGNOSTICS,PUBLIC_SUMMARY,PUBLIC_REPORT,PUBLIC_OVERLEAF,ROOT/"docs/journal/DAY05_BRIEF.md"]
    for path in public:
        text=path.read_text(encoding="utf-8")
        if any(token in text for token in ("subjectid","caseid","C:\\Users\\","patient_profile.json","BEGIN PRIVATE KEY")):raise RuntimeError(f"privacy token in {path}")
