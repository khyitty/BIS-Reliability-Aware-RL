...
def summarize(config_path: Path) -> None:
    ...
    pi_rows=[r for r in csv.DictReader((ROOT/"reports/journal/day04_results.csv").open(encoding="utf-8")) if r["kind"]=="baseline" and r["seed"]=="PI"];pi_mae={r["condition_id"]:float(r["mae_0_1800"]) for r in pi_rows}
    lines=["# Day 06 농도 표현 해부 연구 결과","","## 결론","",f"사전 동결한 표현 유지 규칙에 따라 다음 관측 품질 profile sweep의 단일 추천 상태는 **{diagnosis['selected_state']}**이다. 이 결론은 재구성 PK/PD simulation의 재사용 개발 subset에서 얻은 탐색적 공학 판단이며 통계적 동등성, 임상적 유효성 또는 개별 feature의 생물학적 인과 효과를 뜻하지 않는다.","","## 최종 latent-BIS MAE","","| State | Profile | Seeds 48–52 | Mean ± SD | Median / range | Worst seed/value |","|---|---|---|---:|---:|---:|"]
    for state in config["states"]:
        for p in ("P0","P1"):
            r=by[(p,state)];lines.append(f"| {state} | {p} | "+", ".join(f"{x:.4f}" for x in r["seed_values"])+f" | {r['mean_mae']:.4f} ± {r['seed_sd']:.4f} | {r['median_mae']:.4f} / {r['range']:.4f} | {r['worst_seed']}/{r['worst_seed_mae']:.4f} |")
    ...
    lines += ["","### Seed 50 train-only checkpoint trajectory","","각 셀은 131k → 262k → 393k → 524k checkpoint의 `MAE / propofol mg` 순서다. 이 32-subject training-only 진단은 checkpoint 선택에 쓰지 않았다.","","| State | P0 trajectory | P1 trajectory |","|---|---|---|"]
    for state in config["states"]:
        cells=[]
        for p in ("P0","P1"):
            rows=sorted((r for r in diagnosis["seed50_checkpoint_trajectory"] if r["state"]==state and r["profile"]==p),key=lambda r:r["step"]);cells.append(" → ".join(f"{r['mae_0_1800']:.2f}/{r['total_propofol_mg']:.0f}" for r in rows))
        lines.append(f"| {state} | {cells[0]} | {cells[1]} |")
    ...
    lines += ["","## 인과 가용성 및 guardrails","",audit_data["limitation"]+". Decision-time audit에서 미래/사후 action 입력은 없었고 projection은 dynamics, reward, action timing을 바꾸지 않았다.","",f"전체 final cell 최대 action-boundary={max_boundary:.4f}, core clipping={max_clip:.1f}, actor Tanh saturation={max_tanh:.2e}. S_PROP P1 seed 49는 boundary 0.1061과 action variation 0.8601로 사전 guardrail을 위반했다. 새 최악 실패는 S_REMI seed 51로 P0/P1 MAE 15.8944/18.4544, dose 149.6/128.3 mg, TIR 0.470/0.403이어서 양 profile에 공유된 저투여 패턴이었다.","",f"Day04 tuned PI MAE reference는 P0 {pi_mae['P0']:.4f}, P1 {pi_mae['P1']:.4f}다. Dose, smoothness, TIR, RMSE와 PI-bounded 비교 aggregate는 CSV/JSON에 보존했으며 이는 임상적 우월성 근거가 아니다.","","## Neural Networks 기여와 한계","",f"결과는 {diagnosis['selected_state']}를 후속 관측 신뢰도 실험의 입력 후보로 좁히는 표현 연구다. Feature decomposition만으로 architecture novelty를 확립하지 않으며, 후속의 실제 새 encoder/normalizer 또는 안정화 방법과 untouched reserve 확인이 필요하다. Singleton, model mismatch, 새 encoder는 Day 06에서 시험하지 않았다.","",f"Day04 development-validation subset은 이미 관찰된 개발 자료다. Reserve {protocol['future_reserve_subject_count']} subjects/{protocol['future_reserve_case_count']} cases는 bundle/outcome/statistic access 모두 0이며 historical sealed test access도 0이다.","","## 재현","",f"40 jobs, 20,971,520 timesteps, 160 new + 80 reused checkpoints; one worker/one Torch thread; failures/retries/quarantine 0/0/0. Wall time {timing.get('wall_hours',float('nan')):.2f} h. Resume: `.venv-journal\\Scripts\\python.exe scripts\\journal\\run_day06_concentration_anatomy.py resume`."]
    PUBLIC_REPORT.write_text("\n".join(lines)+"\n",encoding="utf-8")
